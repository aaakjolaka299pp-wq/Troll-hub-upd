...
-- =============================================
--  MAP: nestId → nomor building (1=noob … 12=meowl)
-- =============================================
local NEST_BUILDING_ID = {
    noob             = 1,
    brainrot67       = 2,
    esokSekolah      = 3,
    karkarKurkurkur  = 4,
...
-- =============================================
--  HELPER: masuk nest lewat pintu building (tween)
--  Dipakai di semua bagian GRAB/KLAIM brainrot.
--  tpAndReset() TIDAK diubah (tetap untuk validasi unlock).
-- =============================================
local function tweenEnterNest(nestId)
    local buildingNum = NEST_BUILDING_ID[nestId]
    if not buildingNum or not HRP then return end

    -- Ambil pintu building yang sesuai
    local door
    pcall(function()
        door = workspace.Game.Map.Buildings[tostring(buildingNum)].Interior.Door
    end)
    if not door then return end

    -- Tween ke depan pintu (speed tinggi)
    local doorCF   = CFrame.new(door.Position)
    local speed1   = 120
    local dist1    = (HRP.Position - doorCF.Position).Magnitude
    local t1 = TweenService:Create(
        HRP,
        TweenInfo.new(dist1 / speed1, Enum.EasingStyle.Linear),
        { CFrame = doorCF }
    )
    t1:Play()
    t1.Completed:Wait()
    ...
    -- Tween masuk ke dalam nest
    local targetPos = Vector3.new(-32.9, 34.3, -297.8)
    local high      = 40
    local targetCF  = CFrame.new(targetPos.X, targetPos.Y + high, targetPos.Z)
    local speed2    = 60
    local dist2     = (HRP.Position - targetCF.Position).Magnitude
    local t2 = TweenService:Create(
        HRP,
        TweenInfo.new(dist2 / speed2, Enum.EasingStyle.Linear),
        { CFrame = targetCF }
    )
    t2:Play()
    t2.Completed:Wait()
...
-- =============================================
--  HELPER: tween balik ke base (untuk klaim setelah drop button)
-- =============================================
local function tweenToBase()
    if not HRP then return end
    local targetCF = CFrame.new(BASE_POS)
    local speed    = 60
    ...
    threads.AutoBrainrotV3 = task.spawn(function()
        ...
        while State.AutoBrainrotV3 do
            ...
            if not firstPrompt then
                -- Tidak ada → langsung lanjut ke nest berikutnya (tween di awal loop)
            ...
            if not grabbed then
                ...
                -- Lanjut rotasi nest berikutnya (tween di awal loop)
            else
                ...
                if success then
                    ...
                    notify("Auto Brainrot V3", "✅ Dapat " .. gotName .. "! Door → base...", 3)
                    tweenEnterNest(nestToSearch)
                    tweenToBase()
                    -- Lanjut rotasi nest berikutnya (tween di awal loop)
                else
                    if promptPos then
    ...
    threads.AutoBrainrot = task.spawn(function()
        while State.AutoBrainrot do
            ...
            if activeNest then
                ...
                if not State.AutoBrainrot then break end
                notify("Loop Ulang", "Masuk door nest → tween ke base...", 2)
                tweenEnterNest(activeNest[2])
                tweenToBase()
    ...
    threads.AutoNearest = task.spawn(function()
        while State.AutoNearest do
            ...
            if activeNest then
                ...
                -- Tunggu brainrot spawn (max 5 detik polling tiap 0.1s)
                local spawnWait = 0
                local firstPrompt = nil
                repeat
                    task.wait(0.1)
                    ...
                    while State.AutoNearest do
                        ...
                        if confirmPrompt ~= bestPrompt then
                            ...
                            if not grabbed then
                                ...
                                local success = waitDropButton(3)
                                if success then
                                    notify("Auto Nearest", "Berhasil! Door → base...", 2)
                                    tweenEnterNest(activeNest[2])
                                    tweenToBase()
                                    break
                                else
...
local function startAutoNearestBrainrotV2()
    threads.AutoNearestV2 = task.spawn(function()
        ...
        while State.AutoNearestV2 do
            ...
            -- Tunggu brainrot spawn setelah masuk nest (max 5 detik polling tiap 0.1s)
            local spawnWait = 0
            local firstPrompt = nil
                ...
                while State.AutoNearestV2 do
                    ...
                    if confirmPrompt2 ~= bestPrompt then
                        ...
                        if not grabbed then
                            ...
                            local success = waitDropButton(3)
                            if success then
                                notify("Auto Nearest V2", "Berhasil! Door → base...", 2)
                                tweenEnterNest(nestId)
                                tweenToBase()
                                break
                            else
...
local function startAutoBrainrotV2()
    threads.AutoBrainrotV2 = task.spawn(function()
        ...
        while State.AutoBrainrotV2 do
            ...
            notify("Auto Brainrot V2", success and "Berhasil! Door → base..." or "Timeout, coba lagi...", 2)
            tweenEnterNest(nestId)
            tweenToBase()
            -- Refresh HRP sebelum loop ulang
            Character = LP.Character or Character
            HRP = Character and Character:FindFirstChild("HumanoidRootPart") or HRP
        end
    end)
